...
class Token {
  // add token type and token pattern check
  constructor() {
  ...
  close(s) {
    ...
    if (! c in mapping) throw Error('syntax error, unknown char');
    if (s != mapping[c]) throw Error('syntax error, missing ]');
  }
  add(char) {
    switch (this.type) {
      case 'array':
...
class Tokenizer {
  // convert token to couchbase n1ql field
  constructor(str) {
    this.cur = new Token('');
    this.root = this.cur;
    this.value = str;
  ...
  parse(mod = 'query') {
    ...
    if (mod === 'query') return this.toAnyQuery();
