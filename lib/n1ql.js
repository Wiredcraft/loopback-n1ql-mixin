...
const generateWhere = (where = {}, params) => {
  ...
  const xlike = (key, value, params) => {
    const name = escape(`xlike_${key}_${index()}`);
    const field = getFieldName(key);
    const ql = `ANY array_element IN SUFFIXES(LOWER(${field})) SATISFIES array_element like $${name} END`;
    params[name] = `${value}%`;
    return ql;
  };
  /**
...
module.exports = (Model, bootOptions = {}) => {
  ...
  Model.createIndex = async(index) => {
    ...
    return Promise.map(Object.keys(index), async(name) => {
      ...
      let indexQL = `CREATE INDEX ${name} ON ${bucketName}(${keys}) WHERE _type = ${Model.modelName}`;
